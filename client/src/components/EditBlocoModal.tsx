...
const MODALIDADES_EDITAVEIS: ModalidadePoliciamento[] = [
  "PREL", "DESL", "POST", "PREV", "PE", "FISC", "ESC", "RURAL", "SAT", "REF", "REL",
];

export default function EditBlocoModal({ bloco, onSalvar, onFechar }: EditBlocoModalProps) {
  const [modalidade, setModalidade] = useState<ModalidadePoliciamento>(bloco.modalidade);
  ...
  const handleSalvar = () => {
    ...
    onSalvar({ ...bloco, modalidade, horaInicio, horaFim, local, problemaSolucionar: problema, modusOperandi: modus, acoesPolicia: acoes, observacao });
  };
        ...
        >
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl max-w-2xl w-full max-h-[92vh] overflow-y-auto">
            ...
            <div className="sticky top-0 bg-gradient-to-r from-[#0a2540] to-[#1a3a5c] text-white p-4 flex items-center justify-between rounded-t-2xl">
              <h2 id="edit-bloco-title" className="text-lg font-bold">Editar Bloco</h2>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Hora Início</label>
                  <input
                    type="time"
                    value={horaInicio}
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Hora Fim</label>
                  <input
                    type="time"
                    value={horaFim}
                  ...
                  />
                  {!horaValida && (
                    <p className="text-xs text-red-600 mt-1">Fim deve ser após o início</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <select
                  value={modalidade}
                  onChange={(e) => setModalidade(e.target.value as ModalidadePoliciamento)}
                  className="w-full px-3 py-3 rounded-lg border border-gray-300 text-base focus:outline-none focus:ring-2 focus:ring-[#0a2540]"
                >
              ...
              {/* Problema */}
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Problema a Solucionar</label>
                <textarea
                  value={problema}
              ...
              {/* Modus Operandi */}
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Modus Operandi</label>
                <input
                  type="text"
                  value={modus}
              ...
              {/* Ações */}
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Ações de Polícia</label>
                <input
                  type="text"
                  value={acoes}
